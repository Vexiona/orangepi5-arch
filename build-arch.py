...
def dl(url, output=None):
    print(f"Downloading '{output}' <= '{url}'")
    if output:
        sp.run(['curl', '-qgb', '""', '-fL', '--retry', '3', '--retry-delay', '3', '-o', output, url])
    else:
        sp.run(['curl', '-qgb', '""', '-fL', '--retry', '3', '--retry-delay', '3', url])
    print(f"Downloaded '{output}' <= '{input}'")

def dump_binary_from_repo(url, repo, pkg, local_bin, source_bin): # 1: repo url, 2: repo name, 3: pkgname, 4: local bin, 5: source bin
    dl(url + '/' + repo + '.db', 'cache/repo.db')
    with tarfile.open() 
    desc=tar -xOf cache/repo.db --wildcards "$3"'-*/desc'
    names=sed -n '/%NAME%/{n;p;}' <<< "${desc}"
    case ${#names[@]} in
    0)
